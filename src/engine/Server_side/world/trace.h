...
typedef struct {    // sequence like in [pGame()->trace_allsolid]
...
trace_t SV_MoveDLine(vec3_t start, vec3_t delta, phymovetype_t type, edict_p passedict);
trace_t SV_MoveDBox(vec3_t start, vec3_t delta, phymovetype_t type, edict_p passedict);
