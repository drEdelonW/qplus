...
cString PR_ValueString(etype_t type, eval_p val) {
    ...
    switch (type) {
        ...
        case ev_vector:     snprintf(_line, sizeof(_line), "'%5.1f %5.1f %5.1f'", val->vector.x, val->vector.v[1], val->vector.v[2]);   break;
...
cString PR_UglyValueString(etype_t type, eval_p val) {
    ...
    switch (type) {
        ...
        case ev_vector:     snprintf(_line, sizeof(_line), "%f %f %f", val->vector.x, val->vector.y, val->vector.z);        break;
        default:            snprintf(_line, sizeof(_line), "bad type %i", type);                                            break;
    }
