...
void CL_ParseServerInfo() {
	...
	cl.scores = Hunk_AllocName(
		cl.maxclients * sizeof(*cl.scores),
		"scores"
	);
...
void CL_ParseUpdate(update_bits_t bits) {
	...
	int	i = (bits & U_COLORMAP) ? MSG_ReadByte() : ent->baseline.colormap;
...
void CL_ParseServerMessage() {
	...
	while (1) {
		...
		case svc_clientdata:
			CL_ParseClientdata(MSG_ReadShort());
			break;
