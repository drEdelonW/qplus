...
bool SV_RunThink(edict_p ent) {
    ...
    if ((thinktime <= 0) ||
        (thinktime > sv.time + host_frametime))
        return true;
...
MoveClipFlags_e ClipVelocity(vec3_t in, vec3_t normal, vec3_t out, float overbounce) {
    ...
    if (normal[2] > 0)  blocked |= MOVECLIP_FLOOR;  // floor
    if (!normal[2])     blocked |= MOVECLIP_WALL;  // step
...
MoveClipFlags_e SV_FlyMove(edict_p ent, float time, trace_p steptrace) {
    ...
    for (int bumpcount = 0; bumpcount < numbumps; bumpcount++) {
        if (!ent->v.velocity[0] &&
            !ent->v.velocity[1] &&
            !ent->v.velocity[2]) // ent->v.velocity.is_zero()
            break;
        ...
        if (i != numplanes) { // go along this plane
            ...
            vec3_t dir; CrossProduct(planes[0], planes[1], dir);
...
#ifdef QUAKE2
    float ent_gravity = (ent->v.gravity) ? ent->v.gravity : 1.0;
#else
    eval_p val = GetEdictFieldValue(ent, "gravity");
    float ent_gravity = (val && val->_float) ? val->_float : 1.0;
#endif
...
MoveClipFlags_e SV_TryUnstick(edict_p ent, vec3_t oldvel) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        ent->v.velocity[2] = 0;
        trace_t steptrace;
        MoveClipFlags_e clip = SV_FlyMove(ent, 0.1, &steptrace);
...
void SV_Physics_Client(edict_p ent, int num) {
    ...
    case MOVETYPE_NOCLIP:   VectorMA(ent->v.origin, host_frametime, ent->v.velocity, ent->v.origin);    break;
...
void SV_Physics_Toss(edict_p ent) {
#ifdef QUAKE2
    edict_p groundentity = PROG_TO_EDICT(ent->v.groundentity);
...
#endif
    vec3_t move;    VectorScale(ent->v.velocity, host_frametime, move);
...
void SV_Physics_Step(edict_p ent) {
    ...
    if (!((EntityFlags_t)ent->v.flags & (FL_ONGROUND | FL_FLY | FL_SWIM))) {
        bool hitsound = (ent->v.velocity[2] < sv_gravity.value * -0.1);
...
trace_t SV_Trace_Toss(edict_p ent, edict_p ignore) {
    double save_frametime = host_frametime;
    host_frametime = 0.05;
    ...
    while (1) {
        ...
        vec3_t move;    VectorScale(tent->v.velocity, host_frametime, move);
