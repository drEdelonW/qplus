...
#include "sound.h"
#include "structs/Sound_struct.h"
// #include "Leaf.h"
#include "LeafModel.h"
...
// anything coming from the view entity will allways be full volume
    ...
    // calculate stereo seperation and distance attenuation
        ...
        // snd = ch->sfx;
    vec3_t source_vec = VectorSubtract(ch->origin, listener_origin);
    ...
    ClampLessThen(&ch->rightvol, 0);

    scale = (1.0f - dist) * lscale;
    ch->leftvol = (int)((float)ch->master_vol * scale);
    ClampLessThen(&ch->leftvol, 0);
}
...
void S_UpdateAmbientSounds() {
    ...
    for (int ambient_channel = 0; ambient_channel < NUM_AMBIENTS; ambient_channel++) {
        ...
        if (chan->master_vol < vol) {
            ...
            ClampMoreThen(&chan->master_vol, vol);
        }
        else if (chan->master_vol > vol) {
            chan->master_vol -= (int)((float)host_frametime * ambient_fade.value);
            ClampLessThen(&chan->master_vol, vol);
        }
