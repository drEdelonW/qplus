...
void D_WarpScreen() {   // Under water warp
    ...
    float wratio = w / (float)Scr.canvas.width;
    float hratio = h / (float)Scr.canvas.height;
#warning TODO: remake it to qColor8_p
    for (int v = 0; v < Scr.canvas.height + AMP2 * 2; v++) {
        rowptr[v] = (uint8_p)d_viewbuffer +
            (r_refdef.vrect.y * screenwidth) +
            (screenwidth * (int)((float)v * hratio * h / (h + AMP2 * 2)));
    }

    for (int u = 0; u < Scr.canvas.width + AMP2 * 2; u++) {
        column[u] = r_refdef.vrect.x +
            (int)((float)u * wratio * w / (w + AMP2 * 2));
    }

    turb = intsintable + ((int)(GetClSimTime() * SPEED) & (CYCLE - 1));
    uint8_p dest = vid.frameBuff.pBuff + Scr.canvas.y * Scr.canvas.rowBytes + Scr.canvas.x;

    for (int v = 0; v < Scr.canvas.height; v++, dest += Scr.canvas.rowBytes) {
        col = &column[turb[v]];
        uint8_ar row = &rowptr[v];

        for (int u = 0; u < Scr.canvas.width; u += 4) {
...
void Turbulent8(eSpan_p pspan) {
    ...
    do {
        ...
        ClampInRange(0, &r_turb_st.s, bbextents);

        r_turb_st.t = (int)(dz.tz * z) + tadjust;
        ClampInRange(0, &r_turb_st.t, bbextentt);
        ...
        do {
            ...
            if (count) {
                ...
                ClampInRange(16, &st_next.s, bbextents);    // prevent round-off error on <0 steps from
                //  from causing overstepping & running off the edge of the texture

                st_next.t = (int)(dz.tz * z) + tadjust;
                ClampInRange(16, &st_next.t, bbextentt);    // guard against round-off error on <0 steps
...
void D_DrawSpans8(eSpan_p pspan) {
    ...
    do {
        ...
        ClampInRange(0, &st.s, bbextents);

        st.t = (int)(dz.tz * z) + tadjust;
        ClampInRange(0, &st.t, bbextentt);
        ...
        do {
            ...
            if (count) {
                ...
                st_next.s = (int)(dz.sz * z) + sadjust;
                ClampInRange(8, &st_next.s, bbextents);    // prevent round-off error on <0 steps from
                //  from causing overstepping & running off the  edge of the texture
