...
void R_EntityParticles(r_Entity_p ent) {
    ...
    for (int i = 0; i < NUMVERTEXNORMALS; i++) {
        float angle = GetClSimTime() * _aVelocities[i].x;
        float sy = sinf(angle);
        float cy = cosf(angle);
        angle = GetClSimTime() * _aVelocities[i].y;
        float sp = sinf(angle);
        float cp = cosf(angle);
#if 0 // it was disabled
        angle = GetClSimTime() * _aVelocities[i].z;
        float sr = sinf(angle);
        float cr = cosf(angle);
#endif
    ...
    for (int i = -16; i < 16; i++)
        for (int j = -16; j < 16; j++)
            for (int k = 0; k < 1; k++) {
                ...
                prt->die = GetClSimTime() + 2 + (rand() & 31) * 0.02;
...
void R_RocketTrail(vec3_t start, vec3_t end, RocketTrailType type) {
    ...
    while (len > 0) {
        ...
        case RT_TRACER2: {// tracer
            prt->die = GetClSimTime() + 0.5;
            prt->type = pt_static;
            prt->color = ((type == 3) ? 52 : 230) + (TWICE(tracercount & 4));
