...
void Mod_LoadTexinfo(Lump_p l) {
    ...
    for (int i = 0; i < count; i++, in++, out++) {
        ...
        if (!loadmodel->textures) {
            ...
            if (miptex >= loadmodel->numtextures)       Host_SysError("miptex >= loadmodel->numtextures");
...
void Mod_LoadMarksurfaces(Lump_p l) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (j >= loadmodel->numsurfaces)    Host_SysError("Mod_ParseMarksurfaces: bad surface number");
...
TypeLess_ptr Mod_LoadAllSkins(int numskins, dAliasSkinType_p pskintype) {
    ...
    //  if (!strcmp(loadmodel->name,"progs/player.mdl")) {
            uint8_p texels = Hunk_AllocName(s, _loadName);
