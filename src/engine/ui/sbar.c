...
static int  _scoreboardtop[MAX_SCOREBOARD];
static int  _scoreboardbottom[MAX_SCOREBOARD];
...
void Sbar_DrawScoreboard() {
    ...
    l = _scoreboardlines <= 6 ? _scoreboardlines : 6;

    for (i = 0; i < l; i++) {
        x = 20 * (i & 1);
        y = i / 2 * 8;

        s = &cl.scores[_fragsort[i]];
        if (!s->name[0])
            continue;
