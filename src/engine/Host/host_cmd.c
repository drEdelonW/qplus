...
void Host_Loadgame_f() {
    ...
    fsPathStr_t name; snprintf(name, sizeof(name), "%s/%s", com.gamedir, Cmd_Argv(1));
    COM_DefaultExtension(name, ".sav");
...
int LoadGamestate(cString level, cString startspot) {
    ...
    qPathStr_t mapname;    fscanf(loadGStFile, "%s\n", mapname);
...
void Host_Spawn_f() {
    ...
    if (sv.loadgame) { // loaded games are fully inited allready
        ...
        for (int i = 0; i < NUM_SPAWN_PARMS; i++)
            (&pGame()->parm1)[i] = remoteClient->spawn_parms[i];
        ...
        PR_ExecuteProgram(pGame()->PutClientInServer);
    }
