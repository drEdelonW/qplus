...
void VID_Init(uint8_p palette) {
    ...
    if (UseDisplay) {
        ...
        if (getenv("GSVGAMODE"))
            ...
            current_mode = get_mode(com.argv[COM_CheckParm("-mode") + 1], w, h, d);
        else if (COM_CheckParm("-w") || COM_CheckParm("-h")
            || COM_CheckParm("-d")) {
            if (COM_CheckParm("-w"))
                w = Q_atoi(com.argv[COM_CheckParm("-w") + 1]);
            if (COM_CheckParm("-h"))
                h = Q_atoi(com.argv[COM_CheckParm("-h") + 1]);
            if (COM_CheckParm("-d"))
                d = Q_atoi(com.argv[COM_CheckParm("-d") + 1]);
...
void IN_Init(void) {
    ...
    if (UseMouse) {
        ...
        if (COM_CheckParm("-mdev"))
            mousedev = com.argv[COM_CheckParm("-mdev") + 1];
