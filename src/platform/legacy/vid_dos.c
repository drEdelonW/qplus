...
CVAR_ARC(_vid_default_mode_win, "1");
...
Rgb16_t	d_8to16table[InksNum];	// not used in 8 bpp mode
Rgb24_t	d_8to24table[InksNum];	// not used in 8 bpp mode
...
int VID_SetMode(int modenum, uint8_p palette) {
	...
	Scr.canvas.width = pcurrentmode->width;
	Scr.canvas.height = pcurrentmode->height;
