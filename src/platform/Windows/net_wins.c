...
void WINS_GetLocalAddress() {
    ...
    snprintf(my_tcpip_address, sizeof(my_tcpip_address),
        "%u.%u.%u.%u",
...
int WINS_Init(void) {
    ...
    ((sockaddr_in_p)&broadcastaddr)->sin_port = htons((uint16_t)net_hostport);
...
int WINS_OpenSocket(int port) {
    ...
    Sys_Error("Unable to bind to %s", WINS_AddrToString((qsockaddr_p)&address));
ErrorReturn:
...
cString WINS_AddrToString(qsockaddr_p addr) {
    ...
    snprintf(buffer, sizeof(buffer),
        ...
        (haddr >> 16) & 0xff,
        (haddr >> 8) & 0xff,
        haddr & 0xff, ntohs(((sockaddr_in_p)addr)->sin_port)
    );
    return buffer;
}
...
int WINS_StringToAddr(cString string, qsockaddr_p addr) {
    ...
    ((sockaddr_in_p)addr)->sin_port = htons((uint16_t)hp);
    return 0;
}
...
int WINS_GetNameFromAddr(qsockaddr_p addr, cString name) {
    ...
    hostentry = pgethostbyaddr((int8_p)&((sockaddr_in_p)addr)->sin_addr, sizeof(in_addr_t), AF_INET);
    if (hostentry) {
...
int WINS_GetAddrFromName(cString name, qsockaddr_p addr) {
    ...
    addr->sa_family = AF_INET;
    ((sockaddr_in_p)addr)->sin_port = htons((uint16_t)net_hostport);
...
int WINS_SetSocketPort(qsockaddr_p addr, int port) {
    ((sockaddr_in_p)addr)->sin_port = htons((uint16_t)port);
    return 0;
}
