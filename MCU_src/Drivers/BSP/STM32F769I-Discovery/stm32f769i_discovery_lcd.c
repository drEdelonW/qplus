...
#if defined(USE_LCD_HDMI)
/** DSI timming params used for different HDMI adpater */
typedef struct {
    uint16_t      HACT;
    uint16_t      HSYNC;
    uint16_t      HBP;
...
/** DSI packet params used for different HDMI adpater */
typedef struct {
...
/** LTDC PLL params used for different HDMI adpater */
typedef struct {
    uint16_t      PLLSAIN;
    uint16_t      PLLSAIR;
    uint32_t      PCLK;
...
void BSP_LCD_DrawBitmap(uint32_t Xpos, uint32_t Ypos, uint8_t* pbmp) {
    ...
    if ((bit_pixel / 8) == 4)       InputColorMode = DMA2D_INPUT_ARGB8888;
    else if ((bit_pixel / 8) == 2)  InputColorMode = DMA2D_INPUT_RGB565;
    else                            InputColorMode = DMA2D_INPUT_RGB888;
...
void BSP_LCD_FillPolygon(pPoint Points, uint16_t PointCount) {
    ...
    for (counter = 1; counter < PointCount; counter++) {
        ...
        ClampMoreThen(&IMAGE_LEFT, pixelX);
        ClampLessThen(&IMAGE_RIGHT, pixelX);

        pixelY = POLY_Y(counter);
        ClampMoreThen(&IMAGE_TOP, pixelY);
        ClampLessThen(&IMAGE_BOTTOM, pixelY);
    }
...
/** Returns the ID of connected screen by checking the HDMI
...
static uint16_t LCD_IO_GetID(void) {
    ...
    if (ADV7533_ID == adv7533_drv.ReadID(ADV7533_CEC_DSI_I2C_ADDR)) { return ADV7533_ID; }
    ...
    if (HAL_DMA2D_Init(&hdma2d_discovery) == HAL_OK) {
        if (HAL_DMA2D_ConfigLayer(&hdma2d_discovery, 1) == HAL_OK) {
            if (HAL_DMA2D_Start(&hdma2d_discovery, (uint32_t)pSrc, (uint32_t)pDst, xSize, 1) == HAL_OK) {
                HAL_DMA2D_PollForTransfer(&hdma2d_discovery, 10); /* Polling For DMA transfer */
            }
        }
    }
}
...
static LCD_Driver_t Driver_Type(LCD_Driver_t Lcd_type) {
    /* Read the NT35510 ID */
    if (NT35510_ReadID() == NT35510_ID)         Lcd_type = LCD_CTRL_NT35510;
#if 0
    else {
        /* Read the OTM8009A ID */
        if (OTM8009A_ReadID() == OTM8009A_ID)   Lcd_type = LCD_CTRL_OTM8009A;
