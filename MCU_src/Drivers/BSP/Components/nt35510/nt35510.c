...
uint8_t NT35510_Init(uint32_t ColorCoding, uint32_t orientation) {
    ...
    const uint8_t nt35510_caset_portrait[] = { 0x00, 0x00, (HEIGHT >> 8), (HEIGHT & 0xFF) - 1 ,Cmd_CaSet }; // 479
    const uint8_t nt35510_raset_portrait[] = { 0x00, 0x00, (WIDTH >> 8), (WIDTH & 0xFF) - 1 ,Cmd_RaSet }; // 799
    const uint8_t nt35510_madctl_landscape[] = { Cmd_MadCtl, NT35510_MADCTR_MODE_LANDSCAPE };
    const uint8_t nt35510_caset_landscape[] = { 0x00, 0x00, (WIDTH >> 8), (WIDTH & 0xFF) - 1 ,Cmd_CaSet };// 799
    const uint8_t nt35510_raset_landscape[] = { 0x00, 0x00, (HEIGHT >> 8), (HEIGHT & 0xFF) - 1 ,Cmd_RaSet };// 479
#else
    const uint8_t nt35510_madctl_portrait[] = { Cmd_MadCtl ,NT35510_MADCTR_MODE_PORTRAIT };
    const uint8_t nt35510_caset_portrait[] = { 0x00, 0x00, 0x01, 0xDF ,Cmd_CaSet }; // 479
    const uint8_t nt35510_raset_portrait[] = { 0x00, 0x00, 0x02, 0x7F ,Cmd_RaSet }; // 639
    const uint8_t nt35510_madctl_landscape[] = { Cmd_MadCtl, NT35510_MADCTR_MODE_LANDSCAPE };
    const uint8_t nt35510_caset_landscape[] = { 0x00, 0x00, 0x02, 0x7F ,Cmd_CaSet };// 639
    const uint8_t nt35510_raset_landscape[] = { 0x00, 0x00, 0x01, 0xDF ,Cmd_RaSet };// 479
#endif
