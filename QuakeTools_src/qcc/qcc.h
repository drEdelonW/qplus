...
struct function_s {
    ...
    cString file;  // source file with definition
    int   file_line;
    def_p def;
...
typedef struct {
    ...
    def_p def_tail;  // add new defs after this and move it
...
extern def_p pr_global_defs[MAX_REGS]; // to find def for a global variable
...
type_p PR_ParseType();
cString PR_ParseName();

boolean PR_Check(cString string);
void PR_Expect(cString string);
void PR_ParseError(cString error, ...);


extern jmp_buf  pr_parse_abort;  // longjump with this on parse error
extern int   pr_source_line;
extern cString pr_file_p;
...
void PR_NewLine();
def_p PR_GetDef(type_p type, cString name, def_p scope, boolean allocate);

void PR_PrintDefs();

void PR_SkipToSemicolon();
...
boolean PR_CompileFile(cString string, cString filename);
